...
export const ReduxCard = ({ id }) => {
  // Granular subscription isolating state updates to this component instantiation
  const sensor = useSelector((state) => state.sensors.data[id]);
    ...
    <div className="p-4 bg-slate-900 rounded-xl border border-slate-800 transition-all duration-100 flex flex-col justify-between shadow-lg relative overflow-hidden h-full hover:border-slate-700">
      <div className="flex justify-between items-start">
        ...
        <div className="flex flex-col items-end space-y-1">
          <span className="text-[9px] font-black tracking-widest px-1.5 py-0.5 rounded border font-mono bg-purple-500/10 text-purple-400 border-purple-500/20">
            REDUX
          </span>
          <span 
            ref={counterTextRef}
